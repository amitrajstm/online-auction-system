...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    if (auctions) {
      const nullSellers = auctions.filter((a) => a.seller === null);
      console.log("Auctions with null sellers:", nullSellers.length);
    }
  }, [auctions]);
      ...
      {auctions && auctions.length > 0 ? (
        auctions.map((auction) => (
          <Card
            key={auction._id}
            auction_id={auction._id}
            item_id={auction.seller?._id || "unknown"}
            itemName={auction.itemName}
            itemDescription={auction.itemDescription}
            itemPrice={auction.itemPrice}
            itemPostDate={auction.createdAt}
        ...
        <div className="flex flex-col items-center justify-center py-8 px-4 text-center bg-gray-100 rounded-lg shadow-md my-2 max-w-md max-md:mx-auto">
          ...
          <p className="text-gray-500 mt-2">
            No one has listed any products yet.
          </p>
        </div>
      )}
    </div>
